...
import { getSessionTenantId, getSessionToken } from '@cms/shared-auth';
...
//
// getAuthToken and getTenantId are read per request, so every customFetch call from the
// host *and* from every federated remote carries the current session's bearer token and
// X-Tenant-Id — both libraries are Module Federation singletons, so this single call
