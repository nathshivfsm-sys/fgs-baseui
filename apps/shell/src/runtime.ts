...
//
// getAuthToken, getTenantId, and getCompanyId are read per request, so every customFetch
// call from the host *and* from every federated remote carries the current session's
// bearer token, X-Tenant-Id, and X-Company-Id — both libraries are Module Federation
// singletons, so this single call configures all of them.
