...
That said, `npx shadcn add <component>` does not currently run in this repo at all: it fails resolving the `components`/`ui`/`lib`/`hooks`/`utils` aliases, because those need wildcard filesystem `paths` (e.g. `"@/components/*": ["./src/components/*"]`) to compute where to write files, and `@cms/ui` here maps to a single entry file (`./libs/ui/src/index.ts`) via `tsconfig.base.json`, not a wildcard -- a monorepo/module-boundary structural mismatch, unrelated to Radix vs. Base UI and predating this migration. Making the CLI runnable would mean adding wildcard aliases that risk conflicting with the `@cms/ui/*` deep-import ban in `eslint.config.mjs`, so it hasn't been attempted here. New components in this library are hand-written against the conventions in the six already-migrated ones (see `combobox.tsx` for the most recent example), not scaffolded.
