/** Figma Users listing avatar tints (node 2196:11021). */
...
export const userAvatarClassNames = (
  seed: string | null | undefined,
): { surface: string; foreground: string } => {
  const key = (seed ?? '').trim() || '?';
  const paletteIndex = hashString(key) % USER_AVATAR_PALETTE.length;
  return USER_AVATAR_PALETTE[paletteIndex]!;
};
