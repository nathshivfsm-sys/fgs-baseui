...
`@cms/ui/*` deep imports (e.g. `import { Button } from '@cms/ui/components/ui/button'`) are supported from `apps/*` code, not just inside `libs/ui` -- the `no-restricted-imports` rule that used to block them outside the library is gone. They resolve through the `"./*": "./src/*"` entry in `libs/ui/package.json`. Unlike the barrel, a deep-imported module is **not** a Module Federation singleton: it is bundled into each container that imports it. That is safe here because these are presentational components holding no cross-boundary state, but it is a real difference -- the barrel remains the default for normal usage, and deep imports exist for cases that want a single component without the full `@cms/ui` surface.

This used to be wired differently, and the previous note here flagged an unresolved warning: `@module-federation/vite` logged `Detected alias conflicts with shared modules ... may bypass Module Federation's sharing mechanism` for every `@cms/ui*` entry. **That has now been diagnosed and fixed.** The warning was real: `workspaceAliases()` rewrote the bare specifier to a source path before the plugin could wrap the import in `loadShare`, so `@cms/ui` was never actually shared and each container bundled its own copy -- `singleton: true` notwithstanding. It went unnoticed because nothing in `@cms/ui` carries identity across the boundary; it surfaced only when `@cms/shared-auth` added a React context and remotes began throwing `useAuth must be used within an AuthProvider`. The apps no longer alias `@cms/*` at all (only Storybook does, via `.storybook/aliases.ts`), and the runtime shared scope now shows a single `@cms/ui` provider supplied by the shell. A `'@cms/ui/'` prefix-share entry was also removed: the plugin resolves a prefix share against the package root instead of through `exports`, so it looked for `libs/ui/index.js` and failed the build outright.
