...
### Code health
Three co-equal signals: code health 8.01/10 avg (Good), hotspot health 6.15/10 (improving), worst `libs/ui/src/components/ui/select/select.tsx` at 2.05/10 · maintainability 8.61/10 · performance risk 3 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `tools/integration/src/setup-catalog.integration.test.ts` — change entropy — impact −3.0
- `libs/ui/src/icons/index.ts` — change entropy — impact −2.6
...
- `libs/ui/src/icons/index.ts` — untested hotspot — impact −2.0
